...
//UINT8 SecretHash[] = {0xde, 0xad, 0xbe, 0xef, 0x00, 0xde, 0xad, 0xbe,
//                    0xef, 0x00, 0xde, 0xad, 0xbe, 0xef, 0x00, 0xde,
//                    0xad, 0xbe, 0xef, 0x00, 0xde, 0xad, 0xbe, 0xef,
//                    0x00, 0xde, 0xad, 0xbe, 0xef, 0x00, 0xde, 0xad};
...
static __attribute__((__optimize__("0")))
BOOLEAN
CheckPassword (
  VOID
  )
{
  ...
  Print(L"****************************\n");
  Print(L"*                          *\n");
  Print(L"*   Welcome to the BIOS!   *\n");
  Print(L"*                          *\n");
  Print(L"****************************\n\n");
  
  HashValue = malloc(hash_len);
  ...
  for (; j < 3; j++) {
    ...
    while (TRUE) {
      ...
      if (key.UnicodeChar == 0x0d) {// UTF16 CARRIAGE RETURN (CR) (U+000D)
        password[i] = 0x00;
        Print(L"\n");
        break;
      } else if (i < 140) {
        password[i++] = (CHAR8) key.UnicodeChar;
      }
      Print(L"*");
    }
    Sha256HashAll(password, i, HashValue);
//    if (!CompareMem(HashValue, SecretHash, 32)) {
    if (*((UINT64 *) HashValue) == 0xdeadbeefdeadbeef && 
        *((UINT64 *) HashValue + 8) == 0xdeadbeefdeadbeef &&
        *((UINT64 *) HashValue + 16) == 0xdeadbeefdeadbeef &&
	*((UINT64 *) HashValue + 24) == 0xdeadbeefdeadbeef ) {
      free(HashValue);
      return TRUE;
    } else {
      Print(L"Wrong!!\n");
